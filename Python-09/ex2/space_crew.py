...
class SpaceMission(BaseModel):
	...
	def validate(self):
		...
		if not any(c.rank.value in ["captain", "commander"] for c in self.crew):
			raise ValueError("Must have at least one Commander or Captain")

		if self.duration_days > 365 and sum(c.years_experience for c in self.crew) / len(self.crew) < 5:
			raise ValueError("Long missions (> 365 days) need 50% experienced crew (5+ years)")
		
		if not all(c.is_active == True for c in self.crew):
